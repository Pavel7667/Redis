...
public class TransactionTest extends BaseTest {
    ...
    public void transactionTest(){
        RTransactionReactive trans = this.client.createTransaction(TransactionOptions.defaults());
        RBucketReactive<Long> user1Balance = trans.getBucket("user:1:balance", LongCodec.INSTANCE);
        RBucketReactive<Long> user2Balance = trans.getBucket("user:2:balance", LongCodec.INSTANCE);
        this.transfer(user1Balance, user2Balance, 50)
                .thenReturn(0)
                .map(i -> (5 / i)) // some error
                .then(trans.commit())
                .doOnError(System.out::println)
                .onErrorResume(ex -> trans.rollback())
                .subscribe();
        sleep(1000);
    }
